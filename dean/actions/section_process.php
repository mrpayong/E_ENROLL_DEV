...
try {
    if(isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['submitSection']) && $_POST['submitSection'] === "createSection"){
        ...
        $yr_lvl = isset($_POST['yr_lvl']) ? intVal(trim($_POST['yr_lvl'])) : '';
        ...
        if(empty($program_id) || empty($section) || dataEmptyCheck($yr_lvl)){
            $output['code'] = 501;
        ...
        $new_program = "INSERT INTO class_section (program_id, class_name, year_level, school_year_id, date_modified) VALUES (
            '"      .escape($db_connect, $program_id).      "',
            '"      .escape($db_connect, $section).     "',
            '"      .escape($db_connect, $yr_lvl).      "',
            '"      .escape($db_connect, $key_sy).      "',
            NOW()
        )";
    ...
    if(isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['submitSection']) && $_POST['submitSection'] === "editSection"){
        ...
        $year_level = isset($_POST['new_yr_lvl']) ? trim($_POST['new_yr_lvl']) : "";
        ...
        if(empty($section) || dataEmptyCheck($program_id) || empty($year_level) || dataEmptyCheck($class_id)){
            $output['code'] = 501;
            $output['msg_response'] = "All fields are required.";
            echo json_encode($output);
            exit();
        ...
        $old_data = "";
        $new_data = sha1($class_id . $section . $program_id . $year_level);
        $program_exist = "SELECT class_id, class_name, program_id, year_level FROM class_section WHERE 
        class_id = '".    escape($db_connect, $class_id).   "' ";

        if ($query = call_mysql_query($program_exist)){
            if($data = call_mysql_fetch_array($query)){
                $editId = intVal($data['class_id']);
                $old_data = sha1($data['class_id'] . $data['class_name'] . $data['program_id'] . $data['year_level']);
            } else {
                $output['code'] = 400;
        ...
        $sql_section = "SELECT class_id FROM class_section WHERE 
            class_id = '". escape($db_connect, $class_id)  ."' AND school_year_id = '". escape($db_connect, $school_year_id)    ."'
        ";

...
try {
    if(isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['submitCourse']) && $_POST['submitCourse'] === "createCourse"){
        ...
        $course_limit = isset($_POST['course_limit']) ? intVal(trim($_POST['course_limit'])) : '';
        ...
        if(empty($subject_code) || dataEmptyCheck($unit) || dataEmptyCheck($lec_units) || dataEmptyCheck($lab_units) || dataEmptyCheck($course_limit)){
            $output['code'] = 501;
            $output['msg_response'] = "All fields are required.";
            echo json_encode($output);
            exit();

...
$general_page_title  = 'Admin Uploads';
...
$sql   = "SELECT * FROM user_upload WHERE target_role = 'ADMIN' AND is_archived = 0";
$types = '';
$params = [];
...
<body>
                        ...
                        <div class="col-12">
                            <div class="card upload-card">
                                <div class="card-header text-white fw-semibold d-flex align-items-center justify-content-between flex-wrap" style="background-color: #2563EB; font-size: large;">
                                    <div>
                                        <i class="bi bi-folder-fill"></i>&ensp;Admin Uploads
                                    </div>
                                    <div>
                                ...
                                <div class="card-body mt-3 bg-white">
                                    <form method="get" class="row g-2 align-items-end mb-3">
                                        <div class="col-md-4 col-sm-6">
                                            <label for="admin-date-range" class="form-label mb-1 small">Date range</label>
                                            <input type="text" id="admin-date-range" class="form-control form-control-sm" autocomplete="off">
                                            <input type="hidden" name="from_date" id="admin-from-date-hidden" value="<?php echo htmlspecialchars($fromDate); ?>">
                                            <input type="hidden" name="to_date" id="admin-to-date-hidden" value="<?php echo htmlspecialchars($toDate); ?>">
                                        </div>
                                        <div class="col-md-4 col-sm-6">
                                    ...
                                    <div id="admin-uploads-table"></div>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        window.adminUploadsConfig = {
            tableData: <?php echo json_encode($uploads, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT); ?>,
        };
    </script>
    <script src="<?php echo BASE_URL; ?>admin/js/admin_upload.js?v=<?php echo FILE_VERSION; ?>"></script>
    <script>
        jQuery(function($) {
            ...
            var $input = $('#admin-date-range');
            var $fromHidden = $('#admin-from-date-hidden');
            var $toHidden = $('#admin-to-date-hidden');

document.addEventListener('DOMContentLoaded', function () {
    if (!window.adminLogsConfig) {
        return;
    }

    const tableElement = document.getElementById('admin-log-table');
    if (!tableElement || typeof Tabulator === 'undefined') {
        return;
    }

    const baseUrl = window.adminLogsConfig.baseUrl || '';
    const summaryElement = document.getElementById('admin-log-summary');
    ...
    const table = new Tabulator(tableElement, {
        ...
        ajaxParams: { table: 'activity_log', role: 'admin' },
        ...
        ajaxLoader: true,
        ajaxLoaderLoading: 'Loading admin activity logs...',
        initialSort: [{ column: 'date_log', dir: 'desc' }],
        ajaxResponse: function (url, params, response) {
            // response is expected to be an object: { data: [...], page, last_page, ... }
            updateSummary(response);
            // Let Tabulator handle remote pagination using the full response object
            return response;
        },
        columns: [
            {
    ...
    function reloadWithDateFilter(range) {
        const params = { table: 'activity_log', role: 'admin' };
